  ...
  void _onTabTapped(int index) {
    if (index == 2) {
      Navigator.push(context, MaterialPageRoute(builder: (_) => const UploadScreen()))
          .then((_) => context.read<VideoProvider>().loadLocalVideos());
      return;
    }
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: [BoxShadow(color: AppColors.primary.withValues(alpha: .38), blurRadius: 14, offset: const Offset(0, 5))],
        ),
...
class _FeedScreenState extends State<FeedScreen> {
    ...
    return Consumer<VideoProvider>(
      builder: (context, videos, _) {
        ...
        return Scaffold(
          ...
          body: Stack(children: [
            if (videos.videos.isEmpty) _emptyState(context) else PageView.builder(
            ...
            Positioned(top: 0, left: 0, right: 0, child: SafeArea(child: _topBar(context))),
          ]),
        );
      },
    );
  ...
  Widget _topBar(BuildContext context) {
    return Padding(
      ...
      child: Row(children: [
        IconButton(onPressed: () => _openSettings(context), icon: const Icon(Icons.tune_rounded, color: Colors.white)),
        const Spacer(),
        _tab(context, 'لك', !_following, () => setState(() => _following = false)),
        const SizedBox(width: 22),
        _tab(context, 'أتابع', _following, () => setState(() => _following = true)),
        const Spacer(),
        IconButton(onPressed: () => context.read<VideoProvider>().importVideos(), icon: const Icon(Icons.add_to_photos_rounded, color: Colors.white)),
      ]),
    );
  }
  ...
  Widget _emptyState(BuildContext context) {
    return Center(child: Padding(
      ...
      child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        ...
        const Text('أضف فيديوهاتك المحلية لتشغيلها عموديًا مثل TikTok.', textAlign: TextAlign.center, style: TextStyle(color: Colors.white70, fontSize: 15, height: 1.5)),
        const SizedBox(height: 26),
        FilledButton.icon(onPressed: () => context.read<VideoProvider>().importVideos(), icon: const Icon(Icons.video_library), label: const Text('استيراد فيديوهات')),
      ]),
    ));
  }
  ...
  void _openSettings(BuildContext context) {
    showModalBottomSheet(
      ...
      builder: (_) => const _VideoSettingsSheet(),
    );
  }
}
      ...
      builder: (context, settings, _) {
        return SafeArea(child: Container(
          ...
          child: SingleChildScrollView(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            SegmentedButton<VideoFitMode>(segments: const [
            ...
            ], selected: {settings.fitMode}, onSelectionChanged: (v) => settings.setFitMode(v.first)),
