...
class _SearchScreenState extends State<SearchScreen> {
  ...
  Widget build(BuildContext context) => AnnotatedRegion<SystemUiOverlayStyle>(
    ...
    child: Directionality(textDirection: TextDirection.rtl, child: Scaffold(
      appBar: const TikVplyAppBar(title: 'اكتشف'),
...
class _ThumbState extends State<_Thumb> with AutomaticKeepAliveClientMixin {
  late Future<String?> future;
  @override void initState() { super.initState(); future = VideoThumbnailService.instance.getThumbnailPath(widget.path); }
  @override void didUpdateWidget(covariant _Thumb oldWidget) { super.didUpdateWidget(oldWidget); if (oldWidget.path != widget.path) future = VideoThumbnailService.instance.getThumbnailPath(widget.path); }
  @override Widget build(BuildContext context) { super.build(context); return FutureBuilder<String?>(future: future, builder: (_, s) => s.connectionState == ConnectionState.waiting ? const ColoredBox(color: Color(0xFF101820), child: Center(child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white54))) : s.hasData && s.data!.isNotEmpty ? Image.file(File(s.data!), fit: BoxFit.cover, errorBuilder: (_, __, ___) => _fallback()) : _fallback()); }
