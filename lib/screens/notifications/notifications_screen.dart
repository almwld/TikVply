        ...
        actions: [
          Consumer<NotificationProvider>(
            builder: (_, p, __) => p.unreadCount == 0
                ...
                : TextButton(onPressed: p.markAllAsRead, child: const Text('قراءة الكل')),
          ),
        ],
      ),
            ...
            child: ListView.builder(
              ...
              itemBuilder: (_, i) {
                ...
                return Dismissible(
                  ...
                  onDismissed: (_) => p.deleteNotification(notification.id),
  ...
  Widget _empty(BuildContext context) => Center(
        child: Column(
          ...
          children: [
            ...
            const Text('لا توجد تنبيهات', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w700)),
            const SizedBox(height: 8),
            const Text('ستظهر التفاعلات والمتابعات هنا'),
          ],
        ),
      );
}
...
class _NotificationTile extends StatelessWidget {
    ...
    return Material(
      ...
      child: ListTile(
        ...
        onTap: () async {
          ...
          if (context.mounted) {
            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تم تعليم التنبيه كمقروء')));
          }
        },
      ),
    );
  ...
  IconData _icon(NotificationType t) {
    switch (t) {
      ...
      case NotificationType.share: return Icons.share;
  ...
  String _time(DateTime d) {
    ...
    if (x.inMinutes < 60) return 'منذ ${x.inMinutes} دقيقة';
    if (x.inHours < 24) return 'منذ ${x.inHours} ساعة';
    if (x.inDays < 7) return 'منذ ${x.inDays} يوم';
