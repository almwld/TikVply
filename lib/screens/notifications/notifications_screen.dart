...
class NotificationsScreen extends StatelessWidget {
        ...
        child: Scaffold(
          ...
          body: Consumer<NotificationProvider>(
            builder: (context, provider, _) {
              ...
              if (notifications.isEmpty) return _emptyState(context, provider);
                ...
                child: ListView.separated(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Dismissible(
                      ...
                      child: _NotificationTile(item: item, onTap: () => provider.markAsRead(item.id)),
                    );
                  },
                ),
              );
  ...
  Widget _emptyState(BuildContext context, NotificationProvider provider) {
    return RefreshIndicator(
      onRefresh: provider.loadNotifications,
      child: ListView(
        physics: const AlwaysScrollableScrollPhysics(),
        children: const [
          ...
          SizedBox(height: 14),
          Center(child: Text('لا توجد تنبيهات', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w800))),
          SizedBox(height: 8),
          Center(child: Padding(padding: EdgeInsets.symmetric(horizontal: 32), child: Text('ستظهر هنا الإعجابات والتعليقات والمتابعات عند ربط مصدر التنبيهات الحقيقي.', textAlign: TextAlign.center, style: TextStyle(color: Colors.grey, height: 1.5)))),
        ],
      ),
    );
  }
  ...
  // Kept here as the single icon mapping used by the notification tile.
  Widget _buildIcon(NotificationModel item) => CircleAvatar(
    radius: 24,
...
class _NotificationTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Material(
      color: item.isRead ? Theme.of(context).cardColor : AppColors.primary.withValues(alpha: .06),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(item.message.isEmpty ? item.type.displayName : item.message, style: TextStyle(color: Theme.of(context).textTheme.bodyMedium?.color?.withValues(alpha: .72), height: 1.35)),
                    const SizedBox(height: 4),
                    Text(_relativeTime(item.createdAt), style: const TextStyle(color: Colors.grey, fontSize: 11)),
                  ],
                ),
              ),
              if (!item.isRead)
                const Padding(padding: EdgeInsets.only(top: 7, left: 2), child: DecoratedBox(decoration: BoxDecoration(color: AppColors.primary, shape: BoxShape.circle), child: SizedBox(width: 8, height: 8))),
            ],
          ),
        ),
      ),
