  ...
  List<VideoModel> _prepare(List<VideoModel> source) {
    final filtered = source
        .where(
          (video) =>
              _query.isEmpty ||
              '${video.caption ?? ''} ${video.user?.username ?? ''}'
                  .toLowerCase()
                  .contains(_query),
        )
        .toList();
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              provider.error != null
                  ? 'امنح التطبيق إذن الوسائط ثم أعد المحاولة.'
  ...
  Widget build(BuildContext context) {
    return Shimmer.fromColors(
      ...
      child: GridView.builder(
        ...
        itemBuilder: (_, __) => const ColoredBox(color: Colors.white),
      ),
    );
  }
}
