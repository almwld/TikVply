...
class _VideoPageState extends State<VideoPage> with WidgetsBindingObserver {
  ...
  void _playerListener() {
    ...
    if (!_settings.loop && !_completionSent && duration > Duration.zero && c.value.position >= duration - const Duration(milliseconds: 300)) {
      ...
      _playbackState.removePosition(widget.video.id);
  ...
  Future<void> _applySettings() async {
    ...
    } catch (error) { debugPrint('TikVply video settings failed: $error'); }
  }
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    if (state == AppLifecycleState.paused || state == AppLifecycleState.inactive) {
      if (c != null && c.value.isInitialized) { _savePlaybackPosition(c.value.position); c.pause(); }
    }
  }
  ...
  Widget _playerView() {
    ...
    return GestureDetector(
      ...
      onHorizontalDragEnd: _settings.gesturesEnabled ? (details) {
        final velocity = details.primaryVelocity ?? 0;
        if (velocity.abs() > 300) _seekBy(velocity < 0 ? _settings.skipSeconds : -_settings.skipSeconds);
      } : null,
      child: Center(child: ValueListenableBuilder<VideoPlayerValue>(
        valueListenable: c,
        builder: (_, value, __) {
  ...
  Widget _overlay() => Stack(children: [
    ...
    Positioned(left: 12, right: 12, bottom: MediaQuery.paddingOf(context).bottom + 12, child: ValueListenableBuilder<VideoPlayerValue>(
      valueListenable: _controller!,
      builder: (_, value, __) => !value.isInitialized || !_showControls ? const SizedBox.shrink() : Row(children: [
  ...
  Widget build(BuildContext context) => Shimmer.fromColors(
    ...
    child: Stack(fit: StackFit.expand, children: [
      ...
      Positioned(left: 16, right: 88, bottom: 104, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Container(width: 205, height: 15, decoration: BoxDecoration(color: const Color(0xFF293331), borderRadius: BorderRadius.circular(8))),
        const SizedBox(height: 10),
        Container(width: 145, height: 12, decoration: BoxDecoration(color: const Color(0xFF293331), borderRadius: BorderRadius.circular(8))),
        const SizedBox(height: 9),
        Container(width: 230, height: 9, decoration: BoxDecoration(color: const Color(0xFF293331), borderRadius: BorderRadius.circular(6))),
      ])),
